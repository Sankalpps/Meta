...
- Script: `inference.py` (submission entrypoint)
- Uses OpenAI client with `API_BASE_URL`, `MODEL_NAME`, and `HF_TOKEN`
- Falls back to deterministic heuristic policy if LLM call fails at runtime
- Emits structured logs with `[START]`, `[STEP]`, and `[END]` prefixes
